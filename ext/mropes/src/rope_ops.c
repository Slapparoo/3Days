...
struct mrope_branch_node *mrope_make_branch_node(struct mrope_node *lhs, struct mrope_node *rhs)
{
	...
	if( (node = (struct mrope_branch_node *)TD_MALLOC(sizeof(struct mrope_branch_node))) == NULL)
	{
		return NULL;
	}
...
struct mrope_leaf_node * mrope_make_leaf_node(void)
{
	...
	if( (node = (struct mrope_leaf_node *)TD_MALLOC(sizeof(struct mrope_leaf_node))) == NULL)
	{
		return NULL;
	}
...
mreturn_t mrope_clone_leaf_node(struct mrope_leaf_node *original, struct mrope_leaf_node *clone)
{
	...
	if(cloned_text_weight > 0) {
		...
		cloned_text = (char *)TD_MALLOC(cloned_string_length);
...
mreturn_t mrope_split_leaf_node(struct mrope_leaf_node *original, const size_t index, struct mrope_leaf_node *lhs, struct mrope_leaf_node *rhs)
{
	...
	if(index == 0) {
		...
		rhs_text = (char *)TD_MALLOC(rhs_text_length+1);
		...
		lhs_text = (char *)TD_MALLOC(lhs_text_length+1);
